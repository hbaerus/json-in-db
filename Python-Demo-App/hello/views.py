...
def index():
    ...
    if form.validate_on_submit():
        ...
        s = {}
        for i in ['Profname', 'about', 'age', 'email', 'phone', 'loc', 'empid',
                  'school', 'gradYear', 'involv', 'picture', 'groupd']:
            if request.form[i] != "":
                if i == 'picture':
                    s[i] = _safe_profile_url(request.form[i])
                else:
                    s[i] = request.form[i]
            else:
                s[i] = ''
...
def viewprof(username):
    ...
    results = db_connection.execute(
        """SELECT p.doc FROM user_profile p WHERE p.doc.username = :x""", x=username)
...
def gensearch():
    ...
    if form.validate_on_submit():
        if request.form['searchInput'] != "":
            search_results = db_connection.execute(
                """SELECT p.doc FROM user_profile p WHERE json_textcontains(p.doc, '$', :xvar)""",
                xvar=request.form['searchInput']).fetchall()
        else:
            search_results = db_connection.execute(
                """SELECT p.doc FROM user_profile p""").fetchall()
        results = []
...
def search():
    ...
    if form.validate_on_submit():
        ...
        d = {'Profname': '%', 'about': '%', 'age': '%', 'email': '%',
             'phone': '%', 'loc': '%', 'empid': '%', 'school': '%',
             'gradYear': '%', 'involv': '%', 'groupd': '%'}
        for i in d.keys():
            if request.form[i] != "":
        ...
        search_results = db_connection.execute(
            sql, Profname=d['Profname'].lower(), about=d['about'],
            age=d['age'], email=d['email'], phone=d['phone'], loc=d['loc'],
            groupd=d['groupd'], empid=d['empid'], school=d['school'],
            gradYear=d['gradYear'], involv=d['involv']).fetchall()
        results = []
        for i in range(len(search_results)):
            dict_string = json.loads(search_results[i][0])
            results.append(dict_string)
